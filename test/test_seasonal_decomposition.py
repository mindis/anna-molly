...
class TestSeasonalDecomposition(unittest.TestCase):
    ...
    def test_eval_quantile(self):
        ...
        self.options['params']['error_params']['minimal_upper_threshold'] = 0.94
        error = 0.9
        res = self.test_seasonal_decomposition._eval_quantile(error)
        expect(res).to.be.equal({'flag': 0,
                                 'alpha': 0.1,
        ...
        self.options['params']['error_params']['minimal_upper_threshold'] = 0.96
        error = 0.955
        res = self.test_seasonal_decomposition._eval_quantile(error)
        expect(res).to.be.equal({'flag': 1,
                                 'alpha': 0.1,
    ...
    def test_eval_tukey(self):
        ...
        expect(res).to.be.equal({'flag': 1,
                                 ...
                                 'upper': 1.5})
        # no outlier using minimal thresholds
        self.options['params']['error_params']['minimal_lower_threshold'] = -0.4
        error = -0.3
        res = self.test_seasonal_decomposition._eval_tukey(error)
        expect(res).to.be.equal({'flag': 0,
                                 'lower': -0.4,
    ...
    def test_write(self):
        ...
        self.test_seasonal_decomposition.metric_store.write.assert_called_with('td:service', '[]')
